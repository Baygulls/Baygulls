...
class ImageProcessor():
    ...
    def get_centers(img, thresh, img_threshold_color):
    ...
    def detect_buoys(self, img):
        ...
        g_centers = ImageProcessor.get_centers(img, thresh, img_threshold_green)
        r_centers = ImageProcessor.get_centers(img, thresh, img_threshold_red)
        ...
        g_angles = ImageProcessor.find_angles(g_centers, (res_x, res_y)) # pass in resolution of image to calculate angles
        r_angles = ImageProcessor.find_angles(r_centers, (res_x, res_y))
    ...
    def run(self, auv_state=None):
        ...
        if auv_state['heading'] is not None and self.__camera_type == 'SIM':
        ...
        g_centers, r_centers, g_angles, r_angles = self.detect_buoys(image)
