...
def detect_buoys(img):
    ...
    g_centers = get_centers(thresh, img_threshold_green, img)
    r_centers = get_centers(thresh, img_threshold_red, img)
...
if doPlots:
    fig, ax = plt.subplots()
    for frame_num in range(1627336302, 1627336332):
        ...
        try:
            ...
            for g_center in g_centers:
                ax.plot(g_center[0], g_center[1], 'go')
            for r_center in r_centers:
                ax.plot(r_center[0], r_center[1], 'ro')
            plt.pause(0.1)
            plt.draw()
            # plt.show()
        except:
